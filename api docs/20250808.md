### 2025-09-26 – 초대코드 하드코드 제거(1차) 및 stray 마이그레이션 삭제

- 변경 요약: 런타임 스크립트(`backend/app/scripts/ensure_unlimited_invite_code.py`, `backend/app/scripts/smoke_signup_profile.py`)의 초대코드 기본값 5858 하드코드를 제거하고, 환경설정(UNLIMITED_INVITE_CODE/INVITE_CODE) 기반으로만 동작하도록 통일했습니다. 비정규 경로의 stray 파일 `cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py`는 레포에서 삭제했습니다.
- 검증 결과: Alembic heads 단일성은 변함없음(체인 외 파일 삭제). 스크립트는 미설정 시 즉시 종료하도록 바뀌어 잘못된 기본값 사용을 예방합니다.
- 다음 단계: 테스트/PowerShell 스크립트 및 OpenAPI 예제의 '5858' 문자열을 환경 기반으로 치환하고, 필요 시 `python -m app.export_openapi`로 스키마를 재수출합니다.

### 2025-09-26 – Alembic 외부 stray 마이그레이션 파일 삭제

- 변경 요약: 표준 체인 외 경로(`cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py`)의 stray 파일을 레포에서 삭제했습니다. 공식 체인은 `backend/alembic/versions`만 유지합니다.
- 검증 결과: 코드 참조/실행 경로가 아니어서 런타임/스키마 영향 없음. Alembic heads 단일 정책 유지.
- 다음 단계: 컨테이너 내부에서 `alembic heads` 재확인(1개), 필요 시 `python -m app.export_openapi`로 스키마 재수출(변경 없음으로 생략 가능).
