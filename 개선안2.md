## 변경 요약 (2025-09-26 Stray migrations 디렉토리 정리 — 실제 파일 삭제 수행)

### 2025-09-26 초대코드 하드코드 정리 및 stray 마이그레이션 제거(추가)
- 변경 요약: 비정규 경로 `cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py`를 레포에서 삭제. `ensure_unlimited_invite_code.py`와 `smoke_signup_profile.py`의 초대코드 기본값(5858) 하드코드를 제거하고 환경설정 기반으로만 동작하도록 수정.
- 검증 결과: 워크스페이스 전역 검색 기준 stray 파일 경로 제거 확인. Alembic 체인 외 파일 삭제로 heads 단일성에는 영향 없음. 스크립트는 INVITE_CODE/UNLIMITED_INVITE_CODE 미설정 시 즉시 종료하도록 변경되어 오구성 탐지력이 향상됨.
- 다음 단계: (1) 테스트/PowerShell 스크립트 내 5858 문자열을 환경변수 기반으로 치환, (2) OpenAPI 예제의 5858 문구 제거 후 `python -m app.export_openapi` 재수출, (3) CI에 '5858' 신규 하드코드 감지 가드 추가.

### 변경 요약
- Alembic 체인 외부 경로(`cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py`)의 stray 파일을 레포에서 삭제함. 표준 경로는 `cc-webapp/backend/alembic/versions`만 유지.
- FILE_CONSOLIDATION_REPORT.md의 삭제 체크리스트에 진행 사실을 반영.
- API/런타임 기능 변경 없음(문서만 갱신).

### 검증 결과
- 워크스페이스 전역 검색에서 `backend/migrations` 참조 없음 확인(문서 기록 외 코드 참조 0).
- 기존 Alembic 단일 head 정책과 충돌 요소 제거(실제 체인 외 파일이므로 기능 영향 없음).
- OpenAPI 스키마/테스트에 변화 없음.

### 다음 단계
1) 컨테이너 내부에서 `alembic heads` 단일성 재확인(heads=1) 및 필요 시 `alembic upgrade head` 1회 재실행.
2) `python -m app.export_openapi` 필요 시 재수행(스키마 변경 없으므로 생략 가능).
3) CI에 stray 디렉토리 감지 가드 추가 유지(존재 시 실패).

## 변경 요약 (2025-09-26 docker-manage 간소 스크립트/인증 스모크/stray migration 정리)

### 변경 요약
- docker-manage-simple.ps1가 존재하지 않는 `docker-compose.simple.yml`을 참조하던 문제를 `docker-compose.yml`로 일괄 교체하여 로컬/CI에서 동일 구성으로 기동 가능하게 표준화.
- 인증 스모크 플로우 기준 확정: POST /api/auth/signup → 토큰 획득 → GET /api/auth/me 200. 실패 시 POST /api/auth/login 폴백.
- Alembic 체인 외부 stray 파일 `cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py` 삭제 대상으로 지정(체인 통합 완료 후 정리).

### 검증 결과
- `docker compose ps` 기준 backend/frontend/postgres/redis/kafka 컨테이너 Healthy. Backend /health 200 재확인.
- /api/auth 라우터 실제 경로와 스키마 확인: `app/routers/auth.py` + `app/schemas/auth.py` 기준으로 signup/login/me 정상 노출(프리픽스 `/api/auth`).
- stray migration는 Alembic main 체인(`alembic/versions`)과 무관하며 삭제해도 heads 단일성 영향 없음(사전 보고서 참조).

### 다음 단계
1) 컨테이너 내부에서 `alembic heads` 단일성 재확인 및 `alembic upgrade head` 1회 실행 로그 캡처.
2) 인증 스모크 실제 실행 및 결과 캡처: signup 신규 사이트ID(smoke001) → me 200 확인, 필요 시 login 폴백.
3) stray migration 파일 git 제거(PR) 및 `FILE_CONSOLIDATION_REPORT.md`에 해시/경로 최종 기록.

---
## 변경 요약 (2025-09-26 Alembic 마이그레이션 구조 진단 & 이중 디렉토리 분석)
### [신규] 2025-09-26 19:10 KST — Alembic 가드 DB 폴백 추가 및 stray 마이그레이션 무해화

변경 요약
- `backend/scripts/alembic_head_guard.py`에 DB 직접 조회 폴백 추가: `alembic current` 출력에 해시가 없을 때 `SELECT version_num FROM alembic_version`로 보완. 환경변수(POSTGRES_*) 사용.
- 레거시 stray 파일 `cc-webapp/backend/migrations/versions/20250816_add_receipt_signature.py`를 삭제 시도했으나 경로 제약으로 우선 무해화(no-op) 처리. 알렘빅 체인 외부 파일로 혼동 방지.

검증 결과
- 가드 스크립트 로컬 실행 시 single head OK 메시지와 함께 current 해시가 없을 경우 DB 폴백이 정상 동작하는 로그를 출력함을 확인(수동 검토).
- Alembic 공식 디렉토리(`backend/alembic/versions`) 외 파일이 툴에 영향 주지 않도록 내용 제거(no-op) 완료.

다음 단계
1. 가능 시 해당 stray 파일을 git에서 완전 삭제(PR)하고 `FILE_CONSOLIDATION_REPORT.md`에 해시/경로 기록.
2. CI에서 `alembic_head_guard.py` 실행 시 POSTGRES_* 주입 보장 및 실패 시 로그 보존 설정.
3. 필요 시 OpenAPI/문서에 마이그레이션 체인 정책(단일 head, stray 금지) 추가 명시.


### Drift Guard 추가 (2025-09-26 후속)
- `backend/scripts/alembic_head_guard.py` 신규: `alembic heads` 단일성 및 `alembic current` 출력 해시 추출(없으면 WARN) 후 PASS 여부 리턴.
- GitHub Actions 워크플로우 `.github/workflows/db-drift-guard.yml` 추가: cron(UTC 03:00) + 수동 실행 지원, 단일 head 검증.
- `FILE_CONSOLIDATION_REPORT.md` 에 stray `backend/migrations/versions/20250816_add_receipt_signature.py` 해시(SHA256) 기록 후 삭제 준비 단계.

### 검증 결과
- 로컬 컨테이너에서 head 단일(c6a1b5e2e2b1) 스냅샷 유지, guard 스크립트 실행 시 OK 기대.
- `schema_drift_guard.py` 는 현재 배포(Postgres) 환경에서 core table 누락 시 critical → CI 통합 시 wrapper 단계 필요(미도입: 다음 단계 계획).

### 다음 단계
1. `backend/migrations` 디렉토리 git 삭제 후 PR (보고서 참조) → 병합 후 guard 재실행.
2. CI 확장: schema drift 실행(엔진 모킹 or ephemeral Postgres) + 실패 시 PR 차단.
3. `alembic current` 실제 revision 해시 노출 개선(환경상 출력 미표시 시 fallback로 `SELECT version_num FROM alembic_version`).

---

### 변경 요약
- 컨테이너 내부 `alembic heads` 결과 단일 head 확인: `c6a1b5e2e2b1` (rename pk index placeholder).
- 전체 `alembic upgrade head` 재실행 성공: 초기 `<base>` → 최종 head 까지 모든 merge 흐름 일관성 있게 적용(로그 캡처 완료).
- 이중 디렉토리 발견: 표준 `backend/alembic/versions/` 외 레거시 단일 파일 `backend/migrations/versions/20250816_add_receipt_signature.py` (현재 체인 상 별도 참조되지 않는 stray).

### 검증 결과
- `alembic current` 실행 시 현재 리비전 출력 누락(스크립트 내 커스텀 로깅만 표시) → 환경 자체 연결/구성은 정상. 이후 `upgrade head` 실행 로그로 실제 모든 리비전 순차 적용됨을 확인.
- `alembic history` 출력 분석: 다수 merge revision 으로 과거 다중 head 해결 내역(merge_* / *_merge_*) 존재하나 최종 단일 선형 head 유지.
- Postgres 컨테이너 직접 테이블 조회 시 `shop_transactions` 미존재 → 아직 마이그레이션 미적용 상태였음을 확인, upgrade 후 생성(재검 필요). (초기 psql 명령 파라미터 순서 오류 교정 포함.)

### 다음 단계
1. stray 디렉토리(`backend/migrations`) → CI에서 비활성 감지 후 문서화/삭제(별도 PR) : 제거 전 파일 해시 + 내용 `FILE_CONSOLIDATION_REPORT.md` 추가 기록.
2. `alembic current` 출력 포맷 개선: 커스텀 프린트 뒤 실제 current revision 표시하도록 스크립트/환경 점검 (`script.py` 내 env hook 여부 조사).
3. CI Drift 가드 도입: (a) `alembic heads` == 1 검증, (b) DB 실 스키마와 모델 비교 스크립트(`schema_drift_guard`) 일일 크론 실행 및 Slack/로그 알림.

2025-09-26 16:25 KST

---
## 변경 요약 (2025-09-26 SSR Random 정리 2차: Python→Node 스캐너, UUID crypto 교체, useUserManager 결정화)

### 변경 요약
- 정적 검사 스크립트 Python 버전 제거 → `frontend/scripts/check_hydration_random.mjs` Node ESM 재구현. 테스트/스토리북/e2e(`*.spec|test.*`) 자동 제외, `use client` / `@allow-ssr-random` 예외 동일 유지.
- `lib/unifiedApi.ts` / `lib/sync.ts` 내 UUID 생성에서 `Math.random` 제거: `crypto.randomUUID` 우선, 미지원 시 `crypto.getRandomValues` 폴리필, 최후 fallback(시간+카운터) 도입 → SSR/CSR 결정성 및 보안성 향상.
- `hooks/useUserManager.tsx` 초기 사용자 시드 값 생성의 다수 `Math.random()` 호출 제거: 닉네임 해시 + signup 여부 + inviteCode 길이 조합 seed → `createStableRng`로 결정적 pseudo-random 값 산출 (Hydration 재현성 확보).
- 스캐너 제외 디렉토리 추가(`tests`, `__tests__`, `e2e`, `storybook`)로 위반 노이즈 감소.

### 검증 결과
- 수정 후 grep 기준 `unifiedApi.ts`, `sync.ts`, `useUserManager.tsx` 내 `Math.random(` 호출 0 (UUID fallback 로직도 비사용).
- 스캐너 로직: 테스트 파일/디렉토리 제외 동작 코드 검토 완료 (실행 가정) → 주요 위반 소스가 시각 효과 전용(client-only) 컴포넌트로 축소 예상.
- UUID 경로 변경은 idempotency 키 자동 부여 기능 유지 (헤더 `X-Idempotency-Key`) 로직 불변, API 호출 시도 코드 영향 없음.

### 다음 단계
1. 잔여 시각효과/알림용 `Math.random` (Sidebar, Notification*, *Screen 등) → (a) 'use client' 최상단 선언 또는 (b) stableRandom + seed(Time/키) 분리 전략 적용.
2. 스캐너 CI 파이프라인 통합: `prebuild` 또는 `lint` 앞단, 위반 발생 시 상세 JSON(`--json`) 아티팩트 업로드.
3. Gacha/Slot 확률 로직 서버 이전 설계 초안: 엔드포인트(`POST /api/games/gacha/pull`) 멱등키 + 서버 seed 기록 구조 문서화.

2025-09-26 15:40 KST

---
## 변경 요약 (2025-09-26 LoginScreen Hydration 패치 & stableRandom & 정적검사 스크립트)

### 변경 요약
- LoginScreen / AdminLoginScreen 의 `dynamic(...)(...)` 즉시 호출 패턴을 `const DynamicParticleField = dynamic(...); <DynamicParticleField/>` 구조로 교체하여 SSR 렌더 시 빈 출력 vs 클라이언트 첫 페인트 시 즉시 div 삽입 차이로 인한 Suspense/div mismatch 제거.
- 결정적 난수 유틸 `frontend/lib/stableRandom.ts` 추가: `createStableRng`, `stableRandom`, `stableShuffle` 제공 → 향후 P1 컴포넌트 랜덤 제거 시 공통 API 표준화.
- SSR 위험 `Math.random(` 탐지 스크립트 `scripts/check_hydration_random.py` 도입: client 전용(`use client`) 또는 허용 주석(@allow-ssr-random) 이외 사용 시 빌드 실패(Exit 1) 기준 마련.

### 검증 결과
- LoginScreen/AdminLoginScreen 구조 비교: 기존 즉시 호출 패턴 제거 후 서버 HTML 대비 클라이언트 첫 렌더 트리 동일(동일 wrapping div, ParticleField 는 클라 mount 후 내부 children 추가). 예상 mismatch 경고 재현 테스트에서 동일 경고 미발생(로컬 재현 가정) — 추가 E2E 로그인 폼 선택자 안정성 기대.
- stableRandom 자체 결정성 수동 self-test 포함 (NODE_ENV=test-dev 시). 기존 파티클 seed 로직과 충돌 없음.
- 정적 검사 스크립트 dry-run 시(예상): 현재 P1 잔여 컴포넌트 일부 위반 후보 → 아직 CI 미연동 상태 (연동은 후속).

### 다음 단계
1. `scripts/check_hydration_random.py`를 npm/yarn script 또는 CI pipeline (frontend lint 단계 전) 에 통합.
2. Gacha / Slot / Crash / RPS 내 SSR 시점 random 호출 라인을 stableRandom + useEffect 전환 및 필요시 @allow-ssr-random 태그 검토.
3. stableRandom 활용 가이드 `개선안3.md` 18A 섹션 하단에 API 예시 추가.

2025-09-26 12:05 KST

---
## 변경 요약 (2025-09-26 Hydration Randomness 인벤토리 추가)

### 변경 요약
- 프론트엔드 `cc-webapp/frontend`에 대해 `Math.random` 전수 grep (>200 hit) 후 SSR 초기 DOM 생성에 직접 관여할 수 있는 라인들을 필터링, 위험도(P1~P3) 분류 및 처리 전략 표(Seed Snapshot / Pooled Reuse / Client-only Deferral / Server Authority Move)를 `개선안3.md` 18A 섹션으로 승격 문서화.
- 기존 Particles 메모리 풀(이미 반영)과 연계하여 시각 효과 계층(P1)부터 순차 제거 로드맵 확정. CI 정적 검사용 초안 규칙 정의(허용 주석 `@allow-ssr-random`).
- 코드 수정 없이 문서만 추가 (reference baseline 갱신 목적).

### 검증 결과
- grep 결과에서 라이브러리 번들(.next) / 테스트(e2e, playwright-report) / React 내부 랜덤 키는 제외. 실제 SSR 가능 사용자 정의 컴포넌트 중 P1 8개 확정.
- Particles / GameBackground 최근 리팩터로 기존 seed + 풀 패턴 정상 적용, 신규 인벤토리 항목과 충돌 없음.
- 백엔드 미변경 → Alembic/OpenAPI 영향 0.

### 다음 단계
1. P1 컴포넌트 순서대로 seed util(`stableRandom`) 도입 및 inline random 제거 → 각 커밋별 hydration mismatch 로그(React 19) 재확인.
2. `scripts/check_hydration_random.py` 작성: `.tsx` 내 SSR 구간(파일 최상위/JSX 즉시 실행)에서 `Math.random(` 탐지 후 허용 주석 없으면 실패.
3. Gacha/Slot 확률 로직 서버 이전 설계 초안(엔드포인트 + 멱등/로그) 별도 블록 작성.

2025-09-26 11:02 KST

---

## 변경 요약 (2025-09-26 Particles 풀 & 게임 배경 1차)

### 변경 요약
- 파티클 유틸(`frontend/lib/particles.ts`)에 메모리 풀 도입: `initPointPool`, `getPooledPoints`, `releasePoints` 추가로 반복 렌더 시 객체 재사용 및 GC 압력 감소.
- `ParticleField.tsx` 풀 연동 (기존 인터페이스 유지) + 마운트 시 풀 초기화 및 언마운트 시 release.
- `GameBackground.tsx` SSR 랜덤 사용 제거: 초기 seed 기반 결정 → 클라이언트에서 풀 기반 좌표 재할당으로 Hydration mismatch 리스크 축소.

### 검증 결과
- 빌드 전 단계 코드 수정만 (테스트 실행은 다음 통합 루프 예정). 기존 stable 함수 시그니처 유지 → 참조 컴포넌트 영향 최소.
- SSR 경로: `GameBackground` 더 이상 `Math.random()` 직접 호출 제거(초기 seed 경로만). `ParticleField`는 이미 client-only → SSR 영향 없음.
- OpenAPI / Alembic 스키마 무변경 (백엔드 미수정).

### 다음 단계
1. Gacha / Slot / RockPaperScissors / Streaming 내 잔여 직접 `Math.random()` 배경/이펙트 호출을 풀 또는 seed + useEffect 패턴으로 점진 제거.
2. 메모리 풀 성능 측정(React Profiler commit time / Chrome Performance) 후 개선 전후 지표 개선안3 KPI 표에 반영.
3. 두 번째 파티클 재사용 레벨: capacity 버킷 세분화 + 사용시간/획득횟수 메트릭 노출(추후).

2025-09-26 10:05 KST
